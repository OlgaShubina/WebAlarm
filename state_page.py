        ...
        for graph in self.config.graphic_list:
            ...
            for k,ch in enumerate(self.config.graphics[graph]["name_channel"]):
                ...
                label = self.config.graphics[graph]["translete_name"][k
                        ]+" "+str(x[-1])+" "+self.config.graphics[graph]["units1"]+"\n"+self.config.graphics[graph]["translete_add_inform"]+" "+str(array[0][ch][-1]["value"])+" "+self.config.graphics[graph]["add_units"]
    ...
    def draw_image2(self, array):
        ...
        for graph in self.config.graphic_list:
            for k,ch in enumerate(self.config.graphics[graph]["name_channel"]):
                ...
                index_color+=1
            plt.ylabel(self.config.graphics[graph]["head"]+ ", " +self.config.graphics[graph]["units1"], fontdict=font)
        ...
        with tag('html', lang="en"):
            ...
            with tag('body'):
                with tag('div', ('class', 'head'), id='head'):
                    ...
                    with tag("h2"):
                        text(self.config.head["translete_name"][0]+ " " +str(array[0][self.config.head["name_channel"][0]][-1]["value"])+ " " +self.config.head["units"])
                        ...
                        if(self.config.graphics[graph]["head"]!=""):
                            with tag('table', ('class', 'table')):
                                ...
                                for k, ch in enumerate(self.config.graphics[graph]["name_channel"]):
                                    with tag('tr'):
                                        with tag('td'):
                                            doc.stag("span", klass="color", style="background-color:" + borderColor[index_color]+";")
                                        with tag('td'):
                                            text(self.config.graphics[graph]["translete_name"][k])
                                        with tag('td', ('class', 'name')):
                                            try:
                                                text(str(round(array[0][ch][-1]["value"], self.config.graphics[graph]["round"]))+" ")
                                    ...
                                    if(len(self.config.graphics[graph]["add_inform"])!=0):
                                        with tag('tr'):
                                            ...
                                            with tag('td', ('class', 'name')):
                                                text(str(round(array[0][self.config.graphics[graph]["add_inform"][k]][-1][
                                                        "value"], self.config.graphics[graph]["add_round"])) + " " + self.config.graphics[graph]["add_units"])
                                    index_color+=1
